...
For unit(in reality integration) tests of components we will use enzyme + jest(create react app provide jest behind the hoods).
For unit and integration tests of services, utils we will also use jest :)

Why so much jest? - it simple test runner with build-in jasmine and a lot of other build-in tools, awesome performance and nice UI(in terminal)
For e2e tests, we have not so much choices - selenium based libs or headless chrome based libs. As second one is really young, we will go with selenium based.
There are a lof of choices like - protractor, nightwatc, vanilla, etc. We will select protractor as simple, fast and with big community tool.
